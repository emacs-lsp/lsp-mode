...
*** Performance
    ...
    - Use Emacs 27+ with native json support. (Note: this requires that you have
      [[http://www.digip.org/jansson/][libjansson]] installed, and that emacs was
...
** Supported languages
   ...
   For more details of all supported languages, check the [[https://emacs-lsp.github.io/lsp-mode/page/languages][Languages documentation page]].
...
** FAQ
   ...
   - How do I get flex completion in when using ~Emacs 26~?
     - ~flex~ filtering has been implemented in ~Emacs 27+~ and ~lsp-mode~ is
       using it by default and there is no ~flex~ matching by default. In order
       to have ~flex~ completion by default you could either upgrade to ~Emacs
       27~ or try using ~helm-flex~ implementation including the following
       snippet in your config:
       #+BEGIN_SRC elisp
         (setq lsp-completion-styles '(helm-flex))
       #+END_SRC
       See also [[https://github.com/emacs-lsp/lsp-mode/issues/1652][#1652]].
