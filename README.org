...
** Supported languages
   ...
   | Haxe                  | [[https://github.com/vshaxe/haxe-language-server][haxe-language-server]]                      | Yes            | Either download the vshaxe extension [[https://marketplace.visualstudio.com/items?itemName=nadako.vshaxe][here]] and extract the server.js file in extension/server/bin/, or build the server [[https://github.com/vshaxe/vshaxe/wiki/Installation#from-source][from source]]. Set lsp-clients--haxe-server-path the server.js filepath. |                              |
