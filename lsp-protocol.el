...
(lsp-interface (css:ApplyCodeAction (:arguments) nil))

(lsp-interface (eslint:StatusNotification (:state) nil))
(lsp-interface (eslint:WorkspaceConfigurationReqHandler (:items) nil))
(lsp-interface (eslint:WorkspaceConfiguration (:scopeUri) nil))
(lsp-interface (eslint:OpenDoc (:url) nil))
...
(defconst lsp/call-hierarchy-direction-calls-from 0)
(defconst lsp/call-hierarchy-direction-calls-to 1)
(defconst lsp/response-error-code-parse-error 0)
(defconst lsp/response-error-code-invalid-request 1)
(defconst lsp/response-error-code-method-not-found 2)
(defconst lsp/response-error-code-invalid-params 3)
(defconst lsp/response-error-code-internal-error 4)
(defconst lsp/response-error-code-server-error-start 5)
(defconst lsp/response-error-code-server-error-end 6)
...
(lsp-interface
 ...
 (ServerCapabilities nil (:callHierarchyProvider :codeActionProvider :codeLensProvider :colorProvider :completionProvider :declarationProvider :definitionProvider :documentFormattingProvider :documentHighlightProvider :documentLinkProvider :documentOnTypeFormattingProvider :documentRangeFormattingProvider :documentSymbolProvider :executeCommandProvider :experimental :foldingRangeProvider :hoverProvider :implementationProvider :referencesProvider :renameProvider :selectionRangeProvider :semanticHighlighting :signatureHelpProvider :textDocumentSync :typeDefinitionProvider :typeHierarchyProvider :workspace :workspaceSymbolProvider))
