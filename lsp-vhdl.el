...
(defun lsp-vhdl--create-connection ()
  ...
  (plist-put
   (lsp-stdio-connection (lambda () (list (plist-get params 'server-path) (plist-get params 'server-args))))
   :test? (lambda () (f-executable? (plist-get params 'server-path)))))
...
(defun lsp-vhdl--set-server-path()
  ...
  (cond ((eq lsp-vhdl-server 'hdl-checker) (if (eq lsp-vhdl-server-path nil)
                                               (setq params (plist-put params 'server-path hdl-checker-bin-name))
                                             (setq params (plist-put params 'server-path lsp-vhdl-server-path))))
        ((eq lsp-vhdl-server 'vhdl-tool) (if (eq lsp-vhdl-server-path nil)
                                             (setq params (plist-put params 'server-path vhdl-tool-bin-name))
                                           (setq params (plist-put params 'server-path lsp-vhdl-server-path))))))

(defun lsp-vhdl--set-server-args()
  "Set server arguments based on server selection."
  (cond ((eq lsp-vhdl-server 'hdl-checker) (setq params (plist-put params 'server-args "--lsp")))
        ((eq lsp-vhdl-server 'vhdl-tool) (setq params (plist-put params 'server-args "lsp")))))
